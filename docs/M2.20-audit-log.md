...
**Cumulative across all 15 audit rounds + 9 follow-up commits:** 50 fixes + 16 new tests since `605a2d0`. xUnit: 27 → 35 → 39 → 47 → 51 → 61 → 62 → 64 → **66**; SelfTest: 16/16 stable.
...
- The `install.cmd :do_build` move loop has `>nul` on stderr that could swallow
  ...
  half-registered context-menu verb) but the same pattern. Could be hardened
  in a future round.
